...
**This prohibition is about Unimatrix, not the filesystem.** It bans the `context_*` write
tools above — it does NOT restrict the `Write` tool. Writing `FINDINGS.md` (or the dual-track /
synthesis files) to disk is your **required deliverable**, not a prohibited write. Do it
yourself; do not return findings only in your message and claim you were "blocked from writing."
